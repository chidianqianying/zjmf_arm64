...
1. 首先需要安装php扩展。根据网站要使用的php版本，下载扩展文件（ php7.4），上传到php安装目录 /lib/php/extensions/no-debug-non-zts-xxxx（xxxx为一串数字）文件夹里面。
...
3. 使用 [官方安装包](https://github.com/aazooo/zjmf) 进行安装。填写授权码的时候，随便填写一个的32位大写的MD5字符串，例如可以 [在这里生成](https://www.md5.cz/)。（之前用过官方安装包的，还需要解压此安装包单独覆盖vendor目录）
